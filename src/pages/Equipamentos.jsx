...
const Equipamentos = ({ user }) => {
  ...
  const carregarDados = async () => {
    try {
      ...
      const equipamentosData = equipamentosSnap.docs.map(doc => ({
        id: doc.id,
        ...doc.data()
      }));
  ...
  const equipamentosFiltrados = equipamentos.filter(eq => {
    ...
    const matchCategoria = filterCategoria === 'todos' || eq.categoria === filterCategoria;
  ...
  const stats = {
    ...
    disponiveis: equipamentos.filter(eq => eq.estado === 'disponivel').length,
    emUso: equipamentos.filter(eq => eq.estado === 'em_uso').length,
    manutencao: equipamentos.filter(eq => eq.estado === 'manutencao').length,
    valorTotal: equipamentos.reduce((sum, eq) => sum + (eq.valor || 0), 0),
  ...
  const handleDevolucao = async (equipamentoId) => {
    try {
      const emprestimo = emprestimos.find(emp => emp.equipamentoId === equipamentoId);
          ...
          <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-200">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-blue-600">{stats.emUso}</p>
              </div>
          ...
          <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-200">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-purple-600">{stats.emprestimosAtivos}</p>
              </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-gray-900">
                  {stats.valorTotal.toLocaleString('pt-PT', { style: 'currency', currency: 'EUR' })}
                </p>
              </div>
      ...
      {equipamentosFiltrados.length === 0 ? (
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-12 text-center">
          ...
          <p className="text-gray-600 mb-6">
            {searchTerm || filterCategoria !== 'todos' || filterEstado !== 'todos'
        ...
        <div className={viewMode === 'grid' 
          ...
          {equipamentosFiltrados.map(equipamento => (
            <EquipamentoCard
              ...
              emprestimo={emprestimos.find(emp => emp.equipamentoId === equipamento.id)}
              viewMode={viewMode}
